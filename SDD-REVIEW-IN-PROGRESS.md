...
**Status:** SDD implementation commit `bd1018f` is pushed; the latest source verification passed 110 tests with 3 optional Dashboard skips, along with repository verification, Ruff, and JavaScript checks. Review closeouts are pushed and `main` is synchronized. The implementation SHA passed disposable Hermes profile doctor, CLI init/status, and Desktop-adapter install checks. Consumer integration is pushed and checked; the active profile reconciles to the pinned SDD Git checkout. The requested plugin pin/install is complete. Separate limitations: GUI rendering/accessibility/clipboard acceptance and historical validation findings in consumer `.sdd/`; gateway messaging/HA adapters are disabled by profile policy and outside the pin scope.
**Repository:** `/home/felipe.arantes/Projects/hermes-sdd`
**Branch:** `main` is clean and synchronized with `origin/main`; implementation commit `bd1018fe8cea44dac4a383080b194e7eb24a1c9c` is pushed.
...
- `uv run python -m unittest discover -s tests -v` — earlier **116 tests passed; 3 skipped**. A later full rerun completed with **110 tests passed; 3 skipped**, all optional FastAPI/HTTPX Dashboard tests in the base environment. Current run includes criterion-proof, failed/missing/mismatched evidence, legacy migration, linked-retirement refusal, malformed schema/status, stable-ID, and render-output probes.
- `uv run python -m unittest tests.unit.test_core -v` — latest focused run **78/78 passed**.
